...
| 2026-08-11 | WMS-87 pagination refinement | Changed the location directory to 10 server-paged records on desktop and mobile, disabled repeated navigation while a page is fetching, and added coverage for paging boundaries plus search/filter resets. Focused tests 19/19, full FE tests 195/195, lint, Prettier, production build, and authenticated API smoke against the 15-record warehouse passed with 10 records on page 1 and 5 on page 2. Browser screenshot QA remains blocked by the recorded local `os error 3`. | Completed |
